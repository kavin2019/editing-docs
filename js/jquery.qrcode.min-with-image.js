...
layui.define(['jquery'], function (exports){
    ...
    $.fn.qrcode_core = $.fn.qrcode;
    $.fn.qrcode = function(options){
        var defaults = { render: "canvas", width: 256, height: 256, typeNumber: -1,correctLevel: 2, background: "#ffffff", foreground: "#666", image: null };
        var opts = $.extend({}, defaults, options);

        var ecLevels =['L','M','Q','H'];
        var ecLevelIndex = (opts.correctLevel > 3 ? 0 : opts.correctLevel);
        var optWrapper = { render: opts.render, text: opts.text, size: Math.max(opts.width, opts.height), minVersion: 6, ecLevel: ecLevels[ecLevelIndex], background: opts.background, fill: opts.foreground , image: opts.image, mode: opts.image?4:0 }
        console.log(optWrapper);
        $(this).qrcode_core(optWrapper);
    }
